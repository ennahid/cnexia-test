  ...
  methods: {
    ...
    loadPokemons(limit = null){
        ...
        axios.get(`https://pokeapi.co/api/v2/pokemon?limit=${limit || 15}&offset=${this.pokemonsList.length}`).then(async (res) => {
            const newPokemons = await Promise.all(res.data.results.map(pokemon => {
                const reff = await axios.get(`https://pokeapi.co/api/v2/pokemon/${pokemon.name}`)
                return reff
            }
            // .then((pokemonInfo) => {
            ...
            console.log(newPokemons);
                // this.pokemonsList = [...this.pokemonsList, ...newPokemons.data];
        // this.currentpage++;
        });
    }
